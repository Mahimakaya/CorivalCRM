...
<div id="location_filterbar" class="filterbar" style="display: none;">
    ...
    <div id="country_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT country FROM dental_clinic ORDER BY country";
...
<li class='list-group-item'>
    ...
    <div id="state_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT state FROM dental_clinic ORDER BY state";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
    ...
    <div id="city_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT city FROM dental_clinic ORDER BY city";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
...
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT title FROM dental_clinic ORDER BY title";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT technology FROM dental_clinic ORDER BY technology";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<script>
    $(document).ready(function() {
        ...
        $(".filter_check").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "php/dental_clinic_ajax.php",
                data: {
                    action: action,
                    title: title,
