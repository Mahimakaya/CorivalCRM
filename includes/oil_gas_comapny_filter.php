...
<div id="location_filterbar" class="filterbar" style="display: none;">
    ...
    <div id="country_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT country FROM oil_gas_comapny ORDER BY country";
...
<li class='list-group-item'>
    ...
    <div id="state_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT state FROM oil_gas_comapny ORDER BY state";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
    ...
    <div id="city_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT city FROM oil_gas_comapny ORDER BY city";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
...
<div id="title_filterbar" class="filterbar" style="display:none ;">
    <ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT title FROM oil_gas_comapny ORDER BY title";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<div class="d-grid gap-2">
    <button id="open_employees" class="filterbtn btn">Employees</button>
</div>

<div id="employees_filterbar" class="filterbar" style="display: none;">
Lorem ipsum dolor sit amet consectetur, adipisicing elit. Consectetur sunt nobis, aut architecto neque mollitia ipsam! Ea, qui unde mollitia quod, rerum totam quisquam recusandae veritatis autem ipsam quae quis vitae amet, beatae eos natus eveniet est adipisci voluptate veniam numquam magnam nesciunt. Odit, corporis.
</div>
...
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT technology FROM oil_gas_comapny ORDER BY technology";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<script>
    $(document).ready(function() {
        ...
        $(".filter_check").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "php/oil_gas_comapny_ajax.php",
                data: {
                    action: action,
                    title: title,
