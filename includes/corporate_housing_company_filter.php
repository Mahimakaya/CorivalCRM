...
<div id="location_filterbar" class="filterbar" style="display: none;">
    ...
    <div id="country_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT country FROM corporate_housing_company ORDER BY country";
...
<li class='list-group-item'>
    ...
    <div id="state_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT state FROM corporate_housing_company ORDER BY state";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
    ...
    <div id="city_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT city FROM corporate_housing_company ORDER BY city";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
...
<div id="title_filterbar" class="filterbar" style="display:none ;">
    <ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT title FROM corporate_housing_company ORDER BY title";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<!-- number of units filter starts -->

<div class="d-grid gap-2">
    <button id="open_no_of_units" class="filterbtn btn" style="font-weight: bold;">No of Units</button>
</div>

<div id="no_of_units_filterbar" class="filterbar" style="display:none ;">
...
<div id="technology_filterbar" class="filterbar" style="display: none;">
    <ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT pms FROM corporate_housing_company ORDER BY pms";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<script>
    $(document).ready(function() {
        ...
        $(".filter_check").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "php/corporate_housing_company_ajax.php",
                data: {
                    action: action,
                    title: title,
                    pms: pms,
