...
<div id="location_filterbar" class="filterbar" style="display: none;">
    ...
    <div id="country_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT country FROM pharmaceutical_company ORDER BY country";
...
<li class='list-group-item'>
    ...
    <div id="state_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT state FROM pharmaceutical_company ORDER BY state";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
    ...
    <div id="city_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT city FROM pharmaceutical_company ORDER BY city";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
...
<div id="title_filterbar" class="filterbar" style="display:none ;">
    <ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT title FROM pharmaceutical_company ORDER BY title";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT technology FROM pharmaceutical_company ORDER BY technology";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<!-- hrms filter start here  -->
<div class="d-grid gap-2">
    <button id="open_hrms" class="filterbtn btn">HRMS</button>
</div>

<div id="hrms_filterbar" class="filterbar" style="display: none;">
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT hrms FROM pharmaceutical_company ORDER BY hrms";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
    <div class='form-check'>
        <label for='hrms' class='form-check-label'>
...
<script>
    $(document).ready(function() {
        ...
        $(".filter_check").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "php/pharmaceutical_company_ajax.php",
                data: {
                    action: action,
                    title: title,
