...
<div id="location_filterbar" class="filterbar" style="display: none;">
   ...
   <div class="d-grid gap-2">
    ...
    <div id="country_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT country FROM salons_spa ORDER BY country";
...
<li class='list-group-item'>
    ...
    <div id="state_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT state FROM salons_spa ORDER BY state";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
    ...
    <div id="city_option" class="mt-2" style="display:none ;">
        <ul class="list-group ">
            <?php
            $sql = "SELECT DISTINCT city FROM salons_spa ORDER BY city";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "
<li class='list-group-item'>
...
<div id="title_filterbar" class="filterbar" style="display:none ;">
    <ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT title FROM salons_spa ORDER BY title";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<ul class="list-group">
        <?php
        $sql = "SELECT DISTINCT technology FROM salons_spa ORDER BY technology";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            echo "
<li class='list-group-item'>
...
<script>
    $(document).ready(function() {
        ...
        $(".filter_check").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "php/salon_and_spa_ajax.php",
                data: {
                    action: action,
                    title: title,
